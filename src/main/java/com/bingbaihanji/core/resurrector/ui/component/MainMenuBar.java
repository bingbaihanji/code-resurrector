...
import com.bingbaihanji.core.resurrector.decompiler.DecompilerType;
...
public class MainMenuBar extends JMenuBar {
    ...
    private JCheckBoxMenuItem webServerEnabled;
    ...
    private void buildFileMenu(final JMenu fileMenu) {
        ...
        // 仅为非 OS X 系统添加退出命令。OS X 会自动处理其关闭
        // 操作
    ...
    private void buildOperationMenu(JMenu operationMenu) {
        ...
        webServerEnabled = new JCheckBoxMenuItem("启用 Web 服务");
        webServerEnabled.setSelected(luytenPrefs.isWebServerEnabled());
        webServerEnabled.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                boolean enabled = webServerEnabled.isSelected();
                luytenPrefs.setWebServerEnabled(enabled);

                if (enabled) {
                    CodeResurrector.startWebServerInBackground();
                } else {
                    CodeResurrector.stopWebServer();
                }
            }
        });
        operationMenu.add(webServerEnabled);
    }
