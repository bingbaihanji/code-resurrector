package com.bingbaihanji.core.resurrector.model;

import com.bingbaihanji.core.resurrector.CodeResurrector;
import com.bingbaihanji.core.resurrector.core.ConfigSaver;
import com.bingbaihanji.core.resurrector.core.Keymap;
import com.bingbaihanji.core.resurrector.decompiler.*;
import com.bingbaihanji.core.resurrector.link.DecompilerLinkProvider;
import com.bingbaihanji.core.resurrector.link.LinkProvider;
import com.bingbaihanji.core.resurrector.ui.component.Selection;
import com.bingbaihanji.core.resurrector.view.JFontChooser;
...
public class OpenFile implements SyntaxConstants {
    ...
    public OpenFile(String name, String path, Theme theme, final MainWindow mainWindow) {
        ...
        // Set character encoding to support Chinese display
        // textArea.setDocument(new org.fife.ui.rsyntaxtextarea.RSyntaxDocument()); // Not needed, default document is sufficient
        ...
        // 使用支持中文显示的等宽字体
        String fontName = getSupportedChineseFontName(textArea.getFont().getName());
        textArea.setFont(new Font(fontName, textArea.getFont().getStyle(), luytenPrefs.getFont_size()));
        ...
        scrollPane.addMouseWheelListener(new MouseWheelListener() {
            ...
            public void mouseWheelMoved(MouseWheelEvent e) {
                if (e.getWheelRotation() == 0) {
                    // 这里无需操作。当滚动事件从触控栏
                    // 或 MagicMouse 传递时会发生这种情况。有 getPreciseWheelRotation，但看起来没有
                    // 简单且一致的方式来使用它
                    // 参见 https://github.com/JetBrains/intellij-community/blob/21c99af7c78fc82aefc4d05646389f4991b08b38/bin/idea.properties#L133-L156
                    return;
                }
    ...
    public void setContent(String content) {
        // Ensure content is displayed with correct character encoding
        textArea.setText(content);
        textArea.setCaretPosition(0); // Ensure caret position is correct
    }

    /**
     * 获取支持中文显示的等宽字体名称
     *
     * @param preferredFont 优先字体
     * @return 支持中文的字体名称
     */
    private String getSupportedChineseFontName(String preferredFont) {
        // Windows 中常见的支持中文的等宽字体
        String[] chineseFonts = {
                "Consolas",           // Windows 系统等宽字体，支持中文
                "Microsoft YaHei UI", // 微软雅黑 UI
                "SimSun",             // 宋体
                "NSimSun",            // 新宋体
                "Courier New",        // 常见等宽字体
                "Monospaced"          // Java 默认等宽字体
        };
    ...
    public void decompile() {
        ...
        // 获取当前选择的反编译引擎
        DecompilerType engineType = luytenPrefs.getDecompilerType();

        // For Procyon engine, keep the original logic
        if (engineType == DecompilerType.PROCYON) {
            // Synchronization: don't accept changes from menu during runtime
            ...
            // Use CFR or Vineflower engine for decompilation
            decompileWithAlternativeEngine(engineType);
        }
    }

    private void decompileWithAlternativeEngine(DecompilerType engineType) {
        this.invalidateContent();
        isNavigationLinksValid = false;
        textArea.setHyperlinksEnabled(false);

        try {
            // Get decompiler engine
            IDecompiler decompiler = DecompilerFactory.getDecompiler(engineType);

            // For Procyon engine, special handling is required
            if (decompiler instanceof ProcyonDecompiler) {
                ProcyonDecompiler procyonDecompiler = (ProcyonDecompiler) decompiler;
                // Ensure correct MetadataSystem is used
                if (metadataSystem != null && type != null) {
                    StringWriter stringwriter = new StringWriter();
                    PlainTextOutput plainTextOutput = new PlainTextOutput(stringwriter);
                    plainTextOutput.setUnicodeOutputEnabled(settings.isUnicodeOutputEnabled());
                ...
                // CFR and Vineflower engines use bytecode
                if (type != null) {
                    String typeName = type.getInternalName();
                    // Get bytecode from cache
                    byte[] bytecode = BytecodeCache.getBytecode(typeName);

                    if (bytecode != null) {
                        String result = decompiler.decompileType(typeName, bytecode);
                        setContentPreserveLastScrollPosition(result);
                    } else {
                        String errorMsg = "// Cannot get bytecode: " + typeName + "\n";
                        errorMsg += "// Please confirm the file is loaded from JAR\n";
                        errorMsg += "// Suggest switching back to Procyon engine";
                        setContentPreserveLastScrollPosition(errorMsg);
                    }
                }
            }
            ...
            String errorMsg = "// Decompilation failed: " + e.getMessage() + "\n";
            errorMsg += "// Please try switching to other decompilation engines";
            setContentPreserveLastScrollPosition(errorMsg);
