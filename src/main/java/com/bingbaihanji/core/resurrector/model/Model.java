...
import com.bingbaihanji.core.resurrector.core.LuytenTypeLoader;
import com.bingbaihanji.core.resurrector.decompiler.BytecodeCache;
import com.bingbaihanji.core.resurrector.exception.BinaryFileException;
import com.bingbaihanji.core.resurrector.exception.FileEntryNotFoundException;
import com.bingbaihanji.core.resurrector.exception.FileSizeExceededException;
import com.bingbaihanji.core.resurrector.io.JarEntryFilter;
import com.bingbaihanji.core.resurrector.ui.component.CellRenderer;
import com.bingbaihanji.core.resurrector.util.ResourceCloser;
import com.bingbaihanji.core.resurrector.view.FindBox;
import com.bingbaihanji.core.resurrector.view.MainWindow;
import com.bingbaihanji.core.resurrector.view.panel.WindowManager;
...
public class Model extends JSplitPane {
    ...
    private Set<OpenFile> hmap = Collections.synchronizedSet(new HashSet<>());
    ...
    public Model(MainWindow mainWindow) {
        ...
        try {
            ...
            try {
                CodeResurrector.showExceptionDialog("Exception!", e1);
                String themeXml = LuytenPreferences.DEFAULT_THEME_XML;
                luytenPrefs.setThemeXml(themeXml);
                setTheme(Theme.load(getClass().getResourceAsStream(LuytenPreferences.THEME_XML_PATH + themeXml)));
            } catch (Exception e2) {
                CodeResurrector.showExceptionDialog("Exception!", e2);
            }
        }
        ...
        panel2.setBorder(BorderFactory.createTitledBorder("结构"));
    ...
    public void openEntryByTreePath(TreePath trp) {
        ...
        try {
            ...
            if (trp.getPathCount() > 1) {
                ...
                if (file.getName().endsWith(".jar") || file.getName().endsWith(".zip")) {
                    if (state == null) {
                        ...
                        // 设置字节码缓存
                        BytecodeCache.setCurrentJarFile(jfile);
                    }
                    ...
                    if (entry.getSize() > MAX_UNPACKED_FILE_SIZE_BYTES) {
                        throw new FileSizeExceededException(entry.getSize());
                    }
                ...
                if (file.length() > MAX_UNPACKED_FILE_SIZE_BYTES) {
                    throw new FileSizeExceededException(file.length());
                }
            ...
            CodeResurrector.showExceptionDialog("Unable to open file!", e);
        } finally {
    ...
    public void loadNodesByNames(DefaultMutableTreeNode node, List<String> originalNames) {
    ...
    public void updateTree() {
        com.bingbaihanji.core.resurrector.util.TreeNodeUtils treeUtil = new com.bingbaihanji.core.resurrector.util.TreeNodeUtils(tree);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    CodeResurrector.showExceptionDialog("Cannot open " + file.getName() + "!", e1);
    ...
    private void buildFlatTreeFromMass(List<String> mass) {
        ...
        // 真实包：路径以 classContainingPackageRoot 开始 -> 扁平化
        for (String packagePath : packages.keySet()) {
    ...
    public void closeFile() {
        ...
        if (oldState != null) {
            com.bingbaihanji.core.resurrector.util.ResourceCloser.tryClose(oldState);
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    CodeResurrector.showExceptionDialog("Cannot Navigate!", e);
                } finally {
                    bar.setVisible(false);
                }
            }
    ...
    public static class Tab extends JPanel {
        ...
        public Tab(String title, final Callable<Void> onCloseTabAction) {
            ...
            // 使用统一的图标管理器加载关闭图标
            this.closeButton = new JLabel(com.bingbaihanji.core.resurrector.util.IconResourceManager.loadCloseIcon());
            ...
            closeButton.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    try {
                        ...
                        CodeResurrector.showExceptionDialog("Exception!", ex);
                    }
                }
            });
        }
    ...
    final class State implements AutoCloseable {
        ...
        public void close() {
            ...
            ResourceCloser.tryClose(jarFile);
        }
